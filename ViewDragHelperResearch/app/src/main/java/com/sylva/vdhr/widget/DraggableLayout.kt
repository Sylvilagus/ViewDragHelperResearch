...
class DraggableLayout : RelativeLayout {
    ...
    fun Int.between(e1:Int,e2:Int)=Math.min(e1,e2)<this&&this<Math.max(e1,e2)
    fun Any.le()= Log.e("DraggableLayout",this.toString())
    private interface OperationToChildren {
        fun operate(id: Int?, childProperty: ChildProperty)
    }
    ...
    private val mCallback = object : ViewDragHelper.Callback() {
        ...
        override fun clampViewPositionHorizontal(child: View?, left: Int, dx: Int): Int {
            ...
            val barriers=mChildrenProperties.map{it.value}.
                    filter {
                        intersectResult=childProperty.intersectX(it)
                        it!=childProperty&&it.type!= ChildProperty.Type.INTANGIBLE&&intersectResult>0
                    }
        ...
        override fun clampViewPositionVertical(child: View?, top: Int, dy: Int): Int {
            ...
            val barriers=mChildrenProperties.map{it.value}.
                    filter {
                        intersectResult=childProperty.intersectY(it)
                        it!=childProperty&&it.type!= ChildProperty.Type.INTANGIBLE&&intersectResult>0
                    }

            topFinal= barriers.map {
                when(intersectResult){
                    ChildProperty.INTERSECT_LT->return it.currentY-childProperty.height-1
                    ChildProperty.SUPERPOSE->return it.currentY-childProperty.height-1
                    ChildProperty.INTERSECT_GT->return it.currentY+it.height+1
                    else->top
                }
            }.min()?:top
    ...
    override fun onLayout(changed: Boolean, l: Int, t: Int, r: Int, b: Int) {
        ...
        for (i in 0..childCount - 1) {
            ...
            mChildrenProperties.put(child.id, ChildProperty.fromChild(child, if (i % 2 == 0) ChildProperty.Type.ELASTIC else ChildProperty.Type.TANGIBLE))
        }
    }
